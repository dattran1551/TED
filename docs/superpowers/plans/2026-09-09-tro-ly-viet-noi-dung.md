...
describe('createDb', () => {
  ...
  it('không seed lại nếu bảng thuật ngữ đã có dữ liệu', () => {
    const db = createDb(':memory:')
    db.prepare("UPDATE glossary_rules SET xung_ho = 'đã sửa' WHERE branch = 'hai'").run()

    // Giả lập mở lại app: gọi createDb lần nữa trên cùng 1 file thật sẽ giữ nguyên dữ liệu.
    // Với :memory: mỗi lần gọi là 1 db mới nên ta kiểm tra logic seed qua hàm riêng ở bước sau
    // (kiểm tra tại đây: sau UPDATE, seed không được ghi đè vì bảng đã có dòng).
    const row = db.prepare("SELECT xung_ho FROM glossary_rules WHERE branch = 'hai'").get() as any
